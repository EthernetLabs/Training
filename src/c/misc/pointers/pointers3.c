...
int main(void) {
    ...
    printf("DEBUG>> addr of wet: %p\n", wet);
    printf("DEBUG>> addr of dry: %p\n", dry);
    printf("DEBUG>> cookie[0]: %p\n", cookie[0]);
    printf("DEBUG>> cookie[1]: %p\n", cookie[1]);

    printf("DEBUG>>> *cookie[0]: %.2lf\n", *cookie[0]);
    printf("DEBUG>>> *(cookie[0]+1): %.2lf\n", *(cookie[0]+1));

    printf("DEBUG>>>>wet[0]> **cookie: %.2lf\n", **cookie);
    printf("DEBUG>>>>wet[1]> *(*cookie+1): %.2lf\n", *(*cookie+1));
    printf("DEBUG>>>>dry[0]> **(cookie+1): %.2lf\n", *(*(cookie+1)));
    printf("DEBUG>>>>dry[1]> *(*(cookie+1)+1): %.2lf\n", *(*(cookie+1)+1));
    printf("DEBUG>>>>dry[2]> *(*(cookie+1)+1): %.2lf\n", *(*(cookie+1)+2));

    printf("DEBUG>> *cookie[1]: %.2lf\n", *cookie[1]);
    printf("DEBUG>> *(cookie[1]+1): %.2lf\n", *(cookie[1]+1));
